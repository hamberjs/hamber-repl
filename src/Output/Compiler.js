...
if (is_browser) {
	function worker_fn() {
		...
		self.addEventListener('message', async event => {
			switch (event.data.type) {
				case 'init':
					importScripts(`${event.data.hamberUrl}/compiler.js`);
					fulfil_ready();
					break;
	...
	const blob = new Blob(
		[worker_fn.toString().replace(/^function .+\{?|\}$/g, '')],
...
export default class Compiler {
	constructor(hamberUrl) {
		if (!workers.has(hamberUrl)) {
			const worker = new Worker(url);
			worker.postMessage({ type: 'init', hamberUrl });
		...
		return new Promise(fulfil => {
			...
			this.worker.postMessage({
				...
				options: Object.assign({
					name: component.name,
					filename: `${component.name}.hamber`
				}, options),
